...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("button").setDescription("Send a Button")
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("selectmenu").setDescription("Send a Select Menu")
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("modal").setDescription("Show a Modal")
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("test").setDescription("Send a test button")
    ),
  global: false,
  async execute(interaction) {
    if (interaction.options.getSubcommand() === "button") {
      const row = new ActionRowBuilder().addComponents(
        exampleButton.data.builder
      ); // Button in interactions/exampleButton.js
      ...
      const row = new ActionRowBuilder().addComponents(
        exampleSelectMenu.data.builder
      ); // SelectMenu in interactions/exampleSelectMenu.js
      await interaction.reply({
        content: "Example Select Menu",
        components: [row],
        ephemeral: true,
      });
